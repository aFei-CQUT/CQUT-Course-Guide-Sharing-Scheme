...
class AbsorptionColumn:
    ...
    def __init__(self, gas_flow_rate, so2_mole_fraction,
        ...
        # 水的黏度, Pa·s = 3600 kg/(m·h), 后面计算a_w/a_t时转换成 kg/(m·h)
        self.mu_L = 1.005e-3
        # 空气黏度, Pa·s = 3600 kg/(m·h), 后面计算a_w/a_t时转换成 kg/(m·h)
        self.mu_V = 1.81e-5       
        self.D_L = 1.47e-9        # SO2 在水中的扩散系数, m²/s = 3600 m²/h
        self.D_V = 1.08e-5        # SO2在空气中的扩散系数, m²/s = 3600 m²/h
        # 材料临界表面张力, N/m = kg/s² = 3600² kg/h²
        self.sigma_c = 0.033
        # 水的表面张力, N/m = kg/s² = 3600² kg/h²
        self.sigma_L = 0.0728
        self.g = 9.81             # 重力加速度, m/s² = 3600² m/h²
        # 散装填料的泛点填料因子平均值,计算泛点气速的系数, m^-1
        self.phi_F = 550
        # 散装填料的压降填料因子平均值,计算填料塔压降Eckert图坐标的系数, m^-1
        self.phi_P = 232
        ...
        # Eckert 图的横坐标, 用于泛点气速, 计算时用到参数 Φ_F
        self.X_Eckert_phi_F = None
        # Eckert 图的纵坐标, 用于泛点气速, 计算时用到参数 Φ_F
        self.Y_Eckert_phi_F = None
        # Eckert 图的横坐标, 用于填料层压降, 计算时用到参数 Φ_P
        self.X_Eckert_phi_P = None
        # Eckert 图的纵坐标, 用于填料层压降, 计算时用到参数 Φ_P
        self.Y_Eckert_phi_P = None
        # 结果存储字典
        self.results = {}
        # 计算基础物性参数
    ...
    def compute_liquid_properties(self):
        # 计算液相的相关参数
        # kmol/(kPa·m³),M_L是混合液体的摩尔质量,近似为水的摩尔质量
        self.H = self.rho_L / (self.E * self.M_L)
        # 存储结果
...
def main():
    ...
    # 塔径校核
    flooding_ratio, D_over_d, U = absorption_column.tower_diameter_check(D_rounded,\
                                        absorption_column.d, absorption_column.a_t)
    # 计算传质单元数
    _ = absorption_column.calculate_mass_transfer_units()
    # 计算膜吸收系数
    a_w, k_G, k_L = absorption_column.calculate_membrane_absorption_coefficient\
        (D_rounded, absorption_column.a_t)
    # 计算体积吸收系数
    ...
    if absorption_column.results['Z_prime'] < absorption_column.h_max:
        logger.info(f"设计填料层高度为 {absorption_column.results['Z_prime']:.6f} m,\
                    小于等于最大高度 {absorption_column.h_max} m, 故不进行分段。")
    else:
        logger.info(f"设计填料层高度为 {absorption_column.results['Z_prime']:.6f} m,\
                    超过最大高度 {absorption_column.h_max} m, 需要进行分段。")
